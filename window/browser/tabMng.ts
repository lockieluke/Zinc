...
export default class TabMng {
	...
	public static newTab(url: string): void {
		...
		newtab.addEventListener('animationend', function (event) {
			if (event.animationName === 'url-tab') {
				self.currentURLBar = <HTMLElement>event.currentTarget;
			} else if (event.animationName === 'url-tab-re') {
